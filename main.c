...
int send_string(int serial_fd, const char *msg)
{
    ...
    if (write(serial_fd, &len, sizeof(len)) != sizeof(len))
        return -1; // send length
    if (write(serial_fd, msg, strlen(msg)) != (ssize_t)strlen(msg))
        return -1; // send string
    return 0;
}
#define TESTING
int main()
{
    ...
    while (1)
    {
        ...
        if (read_n(serial_fd, instructions, batch_size * sizeof(uint32_t)) != (ssize_t)(batch_size * sizeof(uint32_t)))
        {
